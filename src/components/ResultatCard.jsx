...
export default function ResultatCard({ dispositif, index }) {
  ...
  return (
    <motion.div
      ...
      style={{ border: '1px solid rgba(26,26,24,0.07)', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.05)' }}
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            <h3 className="font-semibold text-navy text-sm leading-snug">{nom}</h3>
            <span className="shrink-0 text-[10px] font-semibold px-2 py-0.5 rounded-full" style={{ color: '#4CAF7D', background: 'rgba(76,175,125,0.10)', border: '1px solid rgba(76,175,125,0.20)' }}>
              Cumulable
            </span>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {source && (
              <a
                ...
                style={{ color: '#E8915A' }}
                onMouseEnter={e => e.currentTarget.style.color = '#D4724A'}
                onMouseLeave={e => e.currentTarget.style.color = '#E8915A'}
